  ...
  return (
    <div className="relative w-full aspect-video overflow-hidden bg-black/5 shadow-sm">
      ...
      >
        {images.map((img, i) => (
          <img
            ...
            loading="lazy"
            decoding="async"
            className="flex-shrink-0 w-full h-full object-cover will-change-transform transform transition-transform duration-300 ease-[cubic-bezier(.22,.9,.3,1)]"
          />
        ))}
      </div>
      ...
      <Button
        ...
        size="icon"
        onClick={prevImage}
        className="absolute left-3 top-1/2 -translate-y-1/2 bg-white/70 hover:bg-white text-foreground rounded-full shadow-md h-9 w-9 transition-all hover:scale-110"
      ...
      <Button
        ...
        size="icon"
        onClick={nextImage}
        className="absolute right-3 top-1/2 -translate-y-1/2 bg-white/70 hover:bg-white text-foreground rounded-full shadow-md h-9 w-9 transition-all hover:scale-110"
      ...
      >
        <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
              ...
              <div className="absolute bottom-3 sm:bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20">
                {images.map((_, i) => (
                  <button
                    key={i}
                    onClick={() => setIndex(i)}
                    className={`h-1.5 sm:h-2 rounded-full transition-all ${i === index ? "bg-white w-6 sm:w-8" : "bg-white/40 w-2"}`}
                    aria-label={`Go to image ${i + 1}`}
                  />
                ))}
              </div>
          ...
          <button
            ...
            className="absolute top-4 right-4 z-30 rounded-full bg-white/20 hover:bg-white/30 active:bg-white/40 p-2 transition-all duration-200 hover:scale-110"
          >
            <span className="text-white text-xl leading-none">✕</span>
          </button>
        </div>

        <div className="absolute top-4 sm:top-6 left-4 sm:left-6 right-4 sm:right-6 text-center text-xs sm:text-sm text-white/90 pointer-events-none">
...
export function Timeline() {
  ...
  return (
    <section
      ref={ref}
      className="relative min-h-screen flex items-center py-24 px-4 bg-gradient-to-b from-background to-secondary/30"
    >
      {focused !== null && (
        <FocusedViewer milestone={milestones[focused.index]} origin={focused.origin} onClose={closeFocused} />
      )}
      <div className="max-w-6xl mx-auto w-full">
        <h2
          className={`font-cursive text-4xl md:text-6xl font-semibold text-center mb-4 text-primary text-balance ${isVisible ? "animate-fade-in-down" : "opacity-0"}`}
        >
          Our Journey Together
        </h2>
        <p
          className={`text-center text-muted-foreground mb-16 text-lg ${isVisible ? "animate-fade-in-up" : "opacity-0"}`}
        >
          Every moment with you is a memory worth treasuring
        </p>

        <div
          className={`grid gap-6 sm:gap-8 transition-all duration-500 mb-16 ${mounted && isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
          style={{
        ...
        >
          {milestones.map((milestone, index) => (
            <Card
              key={index}
              className={`overflow-hidden transition-all duration-400 transform-gpu rounded-xl bg-gradient-to-br from-white/3 to-white/2 border border-white/6 backdrop-blur-sm cursor-pointer ${
                hoveredKey === `ms-${index}`
                  ? "scale-105 shadow-2xl ring-1 ring-primary/10"
                  : hoveredKey !== null
                    ? "opacity-90"
                    : "hover:shadow-lg hover:ring-1 hover:ring-primary/6"
              } ${isVisible ? `animate-stagger-fade` : ""}`}
              style={{
                transitionDelay: `${index * 30}ms`,
                animationDelay: isVisible ? `${index * 40}ms` : undefined,
              }}
            ...
            >
              <div
                className={`relative overflow-hidden will-change-transform transition-transform duration-400 ${hoveredKey === `ms-${index}` ? "scale-105" : "scale-100"}`}
              >
                {milestone.images ? (
                  ...
                  <div className="w-full aspect-video bg-secondary/50 overflow-hidden">
                    <img
                      ...
                      loading="lazy"
                      decoding="async"
                      className="w-full h-full object-cover transition-transform duration-500 ease-[cubic-bezier(.22,.9,.3,1)] will-change-transform hover:scale-110"
                    />
              ...
              <div className="p-4 sm:p-6 pb-5 sm:pb-7 transition-colors duration-300">
        ...
        <div className="max-w-3xl mx-auto text-center mb-8">
          <h3
            className={`text-lg sm:text-xl font-cursive text-primary mb-1 ${isVisible ? "animate-fade-in-down" : "opacity-0"}`}
          >
            Moving Pictures
          </h3>
          <p className="text-xs sm:text-sm text-muted-foreground">Short clips and candid moments — tap to play.</p>
        </div>
        ...
        >
          ...
          <Card
            className={`bg-card border-primary/6 transition-transform duration-400 transform-gpu rounded-xl overflow-hidden ${
              hoveredKey === "video-1"
                ? "scale-103 shadow-2xl ring-1 ring-primary/10 backdrop-blur-sm"
